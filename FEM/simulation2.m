...
for i=2:steps
 ...
 [ uppernodes,uzip ] = upperdirichletnodes( dp(t), p, U(:,i-1), boundary );

 [fnew Anew Mnew unew] = IncorporateDirichletBoundary(A,M,U(:,i-1),upperNodes,lowerNodes,uzip,uzil);

 U(:,i)= (eye(szU) -(dt^2)/2*Mnew\(fnew-Anew)*U(:,i-1) +dt*dU;
 dU=(1/dt)*(U(:,i)-U(:,i-1));
