...
%     %Time integration:
%     dim = size(Atemp,1);
%     utemp2 = (eye(dim)+0.25*dt^2*Mtemp\Atemp)\(utemp + dt*vtemp-0.25*dt^2*Mtemp\Atemp*utemp+0.5*dt^2*Mtemp\ftemp);
%     utemp2 = putDirichletBack(utemp2,lownodes,upnodes,uz_low,uz_up);
%     U(:,i) = utemp2;
%     v = v -0.5*dt*M\A*(U(:,i)+U(:,i-1));
% end
...
title = 'testDir';
State_to_vtk(output_folder,title,1,sz,tetr(:,1:4),p,U(:,1));
