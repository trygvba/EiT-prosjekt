...
for i=2:steps
      ...
      U(:,i) = K2*(U(:,i-1)+dt*v+0.25*dt^2*K1*U(:,i-1)+0.25*dt^2*(M\(f_vec(p_new,tri,plateForce2(t+dt,omega,OLT),plateDisp(t+dt)) +f_vec(p_new,tri,plateForce2(t,omega,OLT),plateDisp(t)))));
      v = v+0.5*dt*(M\(f_vec(p_new,tri,plateForce2(t+dt,omega,OLT),plateDisp(t+dt)) +f_vec(p_new,tri,plateForce2(t,omega,OLT),plateDisp(t))))+0.5*dt*K1*(U(:,i-1)+U(:,i));
