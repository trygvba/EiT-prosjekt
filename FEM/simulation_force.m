...
omega=pi;                 %Frequency of upperplate.
beta=1/4;
...
for i=2:steps
    t = T0+i*dt;
      ...
      U(:,i) = K2*(U(:,i-1)+dt*v+0.25*dt^2*K1*U(:,i-1)+0.25*dt^2*(M\(f_vec(p,tri,-plateForce(t+dt),plateDisp(t+dt)) +f_vec(p,tri,-plateForce(t),plateDisp(t)))));
      v = v+0.5*dt*(M\(f_vec(p,tri,-plateForce(t+dt),plateDisp(t+dt)) +f_vec(p,tri,-plateForce(t),plateDisp(t))))+0.5*dt*K1*(U(:,i-1)+U(:,i));
