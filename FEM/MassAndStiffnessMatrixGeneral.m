function [A M] = MassAndStiffnessMatrix3D(tet,p,C_vec,rho_vec,phys_groups)
N = size(tet,1);
dim = size(p,1);
for i=1:length(phys_groups)
    Np(i) = sum(tet(:,6)==phys_group(i));
end
...
for k=1:Np(j)
    ...
    for alpha=1:4
        ...
        for beta = 1:4
            ...
            A(i1,j1) = A(i1,j1)+V*a1*C_vec(j)*b1;
            A(i1,j2) = A(i1,j2)+V*a1*C_vec(j)*b2;
            A(i1,j3) = A(i1,j3)+V*a1*C_vec(j)*b3;
            A(i2,j1) = A(i2,j1)+V*a2*C_vec(j)*b1;
            A(i2,j2) = A(i2,j2)+V*a2*C_vec(j)*b2;
            A(i2,j3) = A(i2,j3)+V*a2*C_vec(j)*b3;
            A(i3,j1) = A(i3,j1)+V*a3*C_vec(j)*b1;
            A(i3,j2) = A(i3,j2)+V*a3*C_vec(j)*b2;
            A(i3,j3) = A(i3,j3)+V*a3*C_vec(j)*b3;
